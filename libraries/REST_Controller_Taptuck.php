 ...
 * @version         3.0.0
 */
abstract class REST_Controller_Taptuck extends CI_Controller {
    ...
    protected function _prepare_basic_auth()
    {
        ...
        if($segment != 'taptuck_basic' && $segment != 'cards' && $segment != 'wallet'){
            ...
            if (!empty($auth) && $auth != ''){
                ...
                $this->load->model('user_model');
                $user_id = $this->user_model->get_user_from_token($auth);
            ...
            if($message){
                if($message['success'] == true){
                    $this->response($message, REST_Controller_Taptuck::HTTP_CREATED); // CREATED (201) being the HTTP response code
                }else{
                    $this->response($message, REST_Controller_Taptuck::HTTP_UNAUTHORIZED); //  401
                }
            }
        }
