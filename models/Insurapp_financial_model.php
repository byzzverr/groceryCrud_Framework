...
class Insurapp_financial_model extends CI_Model {
   ...
   function get_all_wallets($date_from='',$date_to=''){
    ...
    if($date_from != '' && $date_to != ''){
      ...
      $query = $this->insurapp_db->query("SELECT u.id as 'user_id', u.name, w.msisdn as 'cellphone', u.email, g.name as 'user_group', sum(w.credit-w.debit) as 'balance' 
   ...
   function get_taptuck_global_transactions(){
      ...
      $query = $this->insurapp_db->query("SELECT w.id, u.name, u.email, w.msisdn, g.name as 'group', w.credit, w.debit, w.reference, w.createdate
        FROM wallet_transactions w  
        LEFT JOIN aauth_users u ON u.username = w.msisdn
        LEFT JOIN aauth_groups g ON u.default_usergroup = g.id 
   ...
   function get_wallet_deposit_stats($msisdn,$date_from='',$date_to=''){
    ...
    $query = $this->insurapp_db->query("SELECT SUM(credit) as 'total', count(id) as 'count' FROM wallet_transactions WHERE msisdn = '$msisdn' AND (reference LIKE '%bank-%' || reference LIKE '%card-%') $date_range");
   ...
   function get_store_rewards($msisdn,$date_from='',$date_to=''){
    ...
    $query = $this->insurapp_db->query("SELECT SUM(credit) as 'total', count(id) as 'count'  FROM wallet_transactions WHERE msisdn = '$msisdn' AND reference LIKE 'reward_store_signup-%' $date_range");
    $result = $query->row_array();
   ...
   function get_wallet_transactions($msisdn, $date_from='',$date_to='', $limit=false){
      ...
      $query = $this->insurapp_db->query("SELECT w.*, u.default_usergroup, u.name FROM wallet_transactions w LEFT JOIN  aauth_users u ON w.msisdn = u.username WHERE  w.msisdn = '$msisdn' order by w.createdate desc $limit");
   ...
   function search_wallet($search_term){
      ...
      $query = $this->insurapp_db->query("SELECT w.*, u.default_usergroup, u.name FROM wallet_transactions w LEFT JOIN  aauth_users u ON w.msisdn = u.username WHERE  w.reference LIKE '%$search_term'");
      $transactions = $query->result_array();
      foreach ($transactions as $key => $value) {
        $transactions[$key]['category'] = $this->categorise_transaction($value);
        if($value['name'] == ''){
   ...
   function store_absa_log($data){
      ...
      $this->insurapp_db->insert('absa_log', $data);
      return $this->insurapp_db->insert_id();
   }

   function get_unprocessed_absa_payments(){

      $query = $this->insurapp_db->query("SELECT * FROM absa_payments WHERE processed = 0");
      return $query->result_array();
   }

   function mark_absa_payment_as_processed($id){
    $this->insurapp_db->query("UPDATE absa_payments SET processed = 1 WHERE id = $id");
   }

   function store_absa_transaction($data){

      $this->insurapp_db->insert('absa_payments', $data);
      return $this->insurapp_db->insert_id();
   }
   ...
   function insert_cref($cref){
    ...
    $this->insurapp_db->insert('paygate',$data);
    return $this->insurapp_db->insert_id();
   }

   function update_cref($data){
    $data->resultdate = date("Y-m-d H:i:s");
    $this->insurapp_db->where('Reference',$data->Reference);
    $this->insurapp_db->update('paygate',$data);
   }
...
/*            //get email message and send
  ...
  function get_rewards($customer_id){
      ...
      $rewards = 0;
      $query = $this->insurapp_db->query("SELECT * FROM `rewards` WHERE customer_id = ?", array($customer_id));
...
/*      $balance = 0;
      $query = $this->insurapp_db->query("SELECT * FROM `customer_accounts` WHERE customer_id = ?", array($customer_id));
      foreach ($query->result_array() as $key => $value) {
...
/*      $balance = 0;
  ...
  function get_wallet_balance($msisdn){
      ...
      $balance = 0;
      $query = $this->insurapp_db->query("SELECT * FROM `wallet_transactions` WHERE msisdn = ?", array(trim($msisdn)));
      foreach ($query->result_array() as $key => $value) {
  ...
  function get_airtime_balance($customer_id){
      ...
      $balance = 0;
      $query = $this->insurapp_db->query("SELECT * FROM `customer_airtime_accounts` WHERE customer_id = ?", array($customer_id));
      foreach ($query->result_array() as $key => $value) {
...
/*  function add_to_taptuck_customer_account($parent, $amount, $reference){
  ...
  function has_task_been_rewarded_before($msisdn, $task_id){
    ...
    $query = $this->insurapp_db->query("SELECT * FROM wallet_transactions WHERE msisdn = '$msisdn' AND reference = '$reference'");
    if($query->num_rows() >= 1){
      return true;
    }
    return false;
  ...
  function masterpass_payment($result){
    ...
    if(isset($result['order_id']) && $result['order_id'] > 0){
      ...
      $message = 'SPAZAPP: Masterpass payment made to the value of R'.$amount.' for order '.$result['order_id'].'.';
      ...
      $message = 'SPAZAPP: Masterpass deposit of R'.$amount.' into your account.';
  ...
  function add_wallet_transaction($type, $amount, $reference, $cellphone){
    ...
    if($result){
      $result = $this->insurapp_db->query("INSERT INTO `wallet_transactions` 
  ...
  function remove_from_customer_account($type, $customer_id, $amount, $order_item_id=0, $added_by=0, $reference=''){
    ...
    switch ($type) {
      ...
      case 'rewards':
        ...
        $this->insurapp_db->query("INSERT INTO `rewards` (customer_id, reward, reason, order_id, createdate) VALUES (?,?,?,?,NOW())", array($customer_id, -$amount, $reference, $order_ref));
        break;
    }
    $data['reference'] = $reference;
    if($transaction_id){
  ...
  function process_taptuck_order($order_id, $type){
    ...
    $query = $this->insurapp_db->query("SELECT o.*, p.id as 'parent_id', k.merchant_id, k.first_name as 'kid_name', m.label as 'product', m.price as 'original_price' FROM tt_orders o, tt_kids k, tt_parents p, tt_menus m WHERE o.menu_id = m.id AND o.kid_id = k.id AND k.parent_id = p.id AND o.id = '$order_id'");
  ...
  {
    $this->insurapp_db->insert("customer_accounts", $data);
  }

  //Credit Taptuck Account
  function credit_taptuck($data)
  {
      $insert = $this->insurapp_db->insert("tt_transactions", $data);
      if($insert)
      {
          return "success";
      }
  ...
  function get_all_trader_wallets($trader_id){
   ...
   function get_electricity_transactions(){
    $query=$this->insurapp_db->query("SELECT w.msisdn, 
                            w.debit, 
                            w.credit, 
                            w.createdate, 
   ...
   function get_taptuck_transactions($type=''){
      if($type=="cashout"){
        $where_reference=" and w.reference='taptuck_cashout'";
        $credit=" ";
      }else{
        $where_reference="";
      ...
      $query = $this->insurapp_db->query("SELECT w.id, u.name, u.email, w.msisdn, g.name as 'group', w.credit, w.debit, w.reference, w.createdate
        ...
        LEFT JOIN aauth_users u ON u.username = w.msisdn
        LEFT JOIN aauth_groups g ON u.default_usergroup = g.id 
        WHERE u.default_usergroup in (14,15) $credit  $where_reference
        ORDER by w.createdate DESC");
      $wallets = $query->result_array();
   ...
   function get_taptuck_cashout_total(){
    $query = $this->insurapp_db->query("SELECT sum(w.credit) as total
        FROM wallet_transactions w  
        LEFT JOIN aauth_users u ON u.username = w.msisdn
        LEFT JOIN aauth_groups g ON u.default_usergroup = g.id 
        WHERE u.default_usergroup in (14,15) and w.reference='taptuck_cashout'
        ORDER by w.createdate DESC");
