...
class Daily_cron extends CI_Controller {
...
//        $distributor_id = 12; // advance
        ...
        //$filename = 'sweetmax template.csv';
        //$distributor_id = 15;
...
/*        if(ENVIRONMENT == 'development'){
            ...
            $incoming = "/var/www/supps365.co.za/absa_docs/netup/NMB00350/incoming/";
            $processed = "/var/www/supps365.co.za/absa_docs/netup/NMB00350/processed/";
        /*}*/
    ...
    function assign_to_account($amount, $reference, $clref){
        ...
        switch ($user['app']) {
            case 'supps365':
                $message = 'SUPPS365: R'.$amount.' has been added to your wallet: '.$clref;
                break;
            case 'spazapp':
...
/*    function force_registration(){
 ...
 function push_notification($template){
  ...
  function test_push_taptuck(){
    ...
    $result = $this->comms_model->test_taptuck_push('e63RgU_swwk:APA91bEVYY-55Sl3nlIScTuy1zPCUGJ8L6hxBoQ6tFdpieXUfNJIDDfbnJUoUYkG6h8GwCklgX810-bcZEvMShn2CRl6TXG5MHF7Mi44mt6LiIhyt6sJqqxLcRV8nCVhNWp6oXeF-4Lc', $body, $title); //jono phone
    print_r($result);
...
function smartcall_batch_query(){

    $result = $this->db->query("SELECT id ticketNumber, clientReference, orderReferenceId FROM smartcall_purchases WHERE ticketNumber = ''");
    $electricity_vouchers = $result->result_array();

    $this->load->library('Smartcall');

    foreach ($electricity_vouchers as $key => $voucher) {
        
        $api_result = $this->smartcall->query($voucher['clientReference']);
...
function split_survey_answers(){

    $run_from = 0;
    $query = $this->db->query("SELECT response_id FROM survey_multi_select_split order by response_id desc limit 1");
    $result = $query->row_array();
    if($result){
        $run_from = $result['response_id'];
    }

    $query = $this->db->query("SELECT a.id, a.text from survey_response_answers a, survey_questions q where a.question_id = q.id and q.question_type = 3 and a.text is not null AND a.id > $run_from order by a.id asc");
    $result = $query->result_array();

    foreach ($result as $key => $answers) {
        if (strpos($answers['text'], ',') !== FALSE) {
            $values = explode(',', $answers['text']);
            foreach ($values as $key => $value) {
                if($this->answer_split_is_unique($answers['id'], $value)){
                    $this->db->insert("survey_multi_select_split", array("response_id" => $answers['id'], "value" => trim($value)));
                }
            }
        }else{
            if($this->answer_split_is_unique($answers['id'], $answers['text'])){
                $this->db->insert("survey_multi_select_split", array("response_id" => $answers['id'], "value" => trim($answers['text'])));
            }
        }
    }
...
function answer_split_is_unique($response_id, $value){

    $query = $this->db->query("SELECT response_id FROM survey_multi_select_split WHERE response_id = $response_id and value = ?", array($value));
    if($query->num_rows() > 0){
        return false;
    }
...
//should run at 12 in the evening.
function order_cleanup(){
    $this->load->model("tt_order_model");
    $this->load->model("tt_merchant_model");
    $merchants = $this->tt_merchant_model->get_tt_merchants();
    $total = 0;
    $cancel = 0;
    $redeem = 0;
    foreach ($merchants as $key => $merchant) {
        $orders = $this->tt_order_model->get_all_merchant_orders($merchant['id'], '', '', false, true);
        foreach ($orders as $key => $order) {

            switch ($order['category']) {
                case 'Meal':
                    $update['status'] = 17;
                    if($this->tt_order_model->update_order($order['order_id'], $update)){
                        $this->financial_model->process_taptuck_order($order['order_id'], 'redeem', false);
                        $redeem += $order['price'];
                    }
                    break;
                case 'Snack':
                case 'Drink':
                    $update['status'] = 14;
                    if($this->tt_order_model->update_order($order['order_id'], $update)){
                        $this->financial_model->process_taptuck_order($order['order_id'], 'cancel', false);
                        $cancel += $order['price'];
                    }
                    break;
            }
