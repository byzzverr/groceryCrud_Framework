...
/*        if (!$this->aauth->is_allowed('Dashboard')){
...

    {
      ...
      public function fridge_log_report(){
        ...
        foreach ($result as $fr) {
         ...
         "'.$fr['street'].'",
         "'.$fr['createdate'].'",
         "'."<li class='dropdown' ><a href='#' class='dropdown-toggle' data-toggle='dropdown' style='color:gray'>Actions  <b class='caret'></b></a><ul class='dropdown-menu' role='menu'><li><a href='/cocacola/dashboard/fridge_locations_history/".$fr['id']."'>Locations History</a></li><li><a href='/cocacola/dashboard/daily_monthly_temperature/".$fr['id']."'>Temperature History</a></li></ul></li></ul>".'"]);';
        }
        $columns=" { title: 'fridge Unit Code' },
                    { title: 'Fridge type' },
                    { title: 'Location Name' },
                    { title: 'Temperature' },
                    { title: 'Province' },
        ...
        $data['script'] =$this->javascript_library->data_table_script($dataset, $columns, 8);
 ...
 function getcurrentlocation($long="-30.086597206537", $lat="30.86423531008"){
    
            $url = 'http://maps.googleapis.com/maps/api/geocode/json?latlng='.trim($long).','.trim($lat).'&sensor=false';
...
public function fridge_details($fridge_id){
        ...
        $data['fridge']=$this->fridge_model->get_recent_locations($fridge_id, 'single row', $data['date_from'], $data['date_to']);
        ...
        $loc_result=$this->fridge_model->get_recent_locations($fridge_id, 'maps', $data['date_from'], $data['date_to']);
      
        foreach ($loc_result as $key => $fridge) {
                ...
                '<br />Region: '.$fridge['region'].
                '<br />Province: '.$fridge['province'].
                '<br />Createdate: '.$fridge['temp_createdate'];
                $marker['animation'] = 'DROP';
                $marker['icon'] = '/assets/img/' . $status['icon'];
        ...
        $temp_result=$this->fridge_model->get_recent_locations($fridge_id, 'chart', $data['date_from'], $data['date_to']);
        foreach ($temp_result as $key => $row) {
                $max_temp2 = $row['expected_temp']+$row['tolerance'];
                $min_temp2 = $row['expected_temp']-$row['tolerance'];
                $off_temp2 = $row['considered_off'];

                $stat = 'Optimal';
                $color = '#27ade5';

                if($row['temp'] > $max_temp2){
                    $stat = 'Too Hot';
                    $color = '#d22c2c';
                }

                if($row['temp'] < $min_temp2){
                    $stat = 'Too Cold';
                    $color = '#d2902d';
                }

                if($row['temp'] > $off_temp2){
                    $stat = 'Fridge is Off';
                    $color = '#d22c2c';
                }

                $labels .= $comma.'"'.humanize($row['temp_createdate']." [".$stat."]").'"';
        ...
        $temperatures=$this->fridge_model->get_recent_locations($fridge_id, 'table', $data['date_from'], $data['date_to']);
        foreach ($temperatures as $key => $value) {
                $max_temp3 = $value['expected_temp']+$value['tolerance'];
                $min_temp3 = $value['expected_temp']-$value['tolerance'];
                $off_temp3 = $value['considered_off'];
                $stat2 = 'Optimal';

                if($value['temp'] > $max_temp3){
                    $stat2 = 'Too Hot';
                }

                if($value['temp'] < $min_temp3){
                    $stat2 = 'Too Cold';
                }

                if($value['temp'] > $off_temp3){
                    $stat2 = 'Fridge is Off';
                }
              $dataset.='dataSet.push([
                         "'.$value['location_name'].'",
                         "'.$value['fridge_unit_code'].'",
                         "'.$value['temp'].'&deg'.'",
                         "'.$stat2.'",
                         "'.$value['temp_createdate'].'"
                         ]);'; 
        }

        $columns="{ title: 'Temperature' },
                    { title: 'Location Name' },
                    { title: 'Fridge Unit Code' },
                    { title: 'Status' },
                    { title: 'Createdate' }";

        $data['script'] =$this->javascript_library->data_table_script($dataset, $columns, 4, false, false, $data['fridge']['location_name'].' -  Temperature Log').
        $this->javascript_library->colored_bar_chart_script($labels, $values, $barColors);
